...
const converter = core_1.getColorConverter("sRGB", "sRGB");
function addPalette(name, type, ...colors) {
    exports.predefinedPalettes.push({
        name,
        type,
        colors: colors.map((x) => x.map((y) => {
            const c = core_1.colorFromHTMLColor(y);
...
function addPowerBIThemeColors() {
    const amount = 60;
    const colors = core_1.getDefaultColorPalette(amount);
    if (colors.length == amount) {
        ...
        const resetFn = core_1.getDefaultColorGeneratorResetFunction();
        if (resetFn) {
            resetFn();
        }
    }
...
function addColorPalette(colorPalette) {
    if (!flag) {
        exports.predefinedPalettes.push(Object.assign({}, colorPalette));
        flag = true;
    }
}
