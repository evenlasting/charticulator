...
class PolarCoordinates extends CoordinateSystem {
    ...
    transformDirectionAtPoint(angle, radial, dx, dy) {
        const t = common_1.Geometry.degreesToRadians(-angle);
        return {
...
class CoordinateSystemHelper {
    ...
    rect(x1, y1, x2, y2, style = {}, rx = 0, ry = 0) {
        ...
        if (cs instanceof BezierCurveCoordinates) {
            if (Math.abs(x1 - x2) < 1e-6) {
                ...
                for (let i = 1; i <= segments; i++) {
                    ...
                    const angle1 = common_1.Geometry.degreesToRadians(framePrevious.angle) + direction;
                    const angle2 = common_1.Geometry.degreesToRadians(frame.angle) + direction;
